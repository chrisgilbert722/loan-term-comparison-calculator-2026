...
import { calculateLoanTermComparison } from './logic/loanTermComparisonCalculations';
import type { LoanTermComparisonInput } from './logic/loanTermComparisonCalculations';

function App() {
  const [values, setValues] = useState<LoanTermComparisonInput>({
    loanAmount: 25000,
    apr: 7.5
  });

  const handleChange = (field: keyof LoanTermComparisonInput, value: number | boolean) => {
    setValues(prev => ({ ...prev, [field]: value as number }));
  };

  const result = calculateLoanTermComparison(values);
